...
<body>
	...
	<h4>社员检索画面</h4>
		<div id="lockOne">
			...
			<c:forEach items="${gdvUserList }" var="items">
				<tr>
					<td>${items.lnkUserCd}
						<input type="radio" id="defualtAdd" name="lnkUserCd" onclick="toUser()" value="${items.lnkUserCd}">
					</td>
					<td>${items.lblFullName}</td>
					<td>${items.lblBranch}</td>
					<td>${items.lblSect}</td>
					<td>${items.lblPostCls}</td>
				</tr>
			</c:forEach>
		</table>
	</div>
